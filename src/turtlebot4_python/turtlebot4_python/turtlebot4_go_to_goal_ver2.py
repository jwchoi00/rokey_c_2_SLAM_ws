...
class GoalSubscriberNode(Node):
    ...
    def goal_callback(self, msg):
        if not self.goal_in_progress:
            ...
            # Convert the Point message into NavigateToPose goal and send it
    ...
    def send_goal_to_robot(self, goal):
        ...
        goal_msg.pose.header = Header(stamp=self.get_clock().now().to_msg(), frame_id="map")
        goal_msg.pose.pose.position.x = goal[0]
        goal_msg.pose.pose.position.y = goal[1]
        goal_msg.pose.pose.position.z = 0.0  # Set to 0 for 2D navigation
